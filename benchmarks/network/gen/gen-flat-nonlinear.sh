...
outputMode(){
    ...
    let "GEN_ON=`currentStateOfGenerator`"
      tanks_pouring=""
    for((i=1; i<=$DIMENSION; i++)) do {
	tank_pouring=`currentStateOfTanki $i`
	if [ $tank_pouring -eq "2" ] ; then
	    tank_rate="(0.1 * (tank"$i"_refuel_time ^ 2))"
	    #echo "$tank_rate"
	    if [ "$tanks_pouring" == "" ] ; then
		tanks_pouring=$tank_rate
	    else
		tanks_pouring=$tanks_pouring" + "$tank_rate
	    fi
	fi
    };done
...
#	if [ `currentStateOfTanki $i` -eq "1" ]; then
...
for((i=1; i<=$DIMENSION; i++)) do {
echo "[0, 1000] tank"$i"_refuel_time;"
}; done
echo "[0, 1000] generator_time;"
echo "[0, 1000] time;"
#echo "[0, 1001] clock;"
