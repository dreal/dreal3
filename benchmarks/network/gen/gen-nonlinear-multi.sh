...
component_refuel_gen_tank (){
    INSTANCE=$1
    TANK_DURATION=11
    RATE="(0.1 * (T_refuel_gen_tank1_automaton * T_refuel_gen_tank1_automaton))"
...
component_lock() {
   ...
   printf "(did_start_gen): (true) ==> @unlocked (true);\n\
	...
	jump:\n "
   for((i = 0; i < $DIMENSION; i++)); do {
       printf "(do_start_refuel$i): (true) ==> @locked (true);\n\
	(do_stop_refuel$i): (true) ==> @locked (true);\n "
       }; done
   printf "(do_stop_gen): (true) ==> @locked (true);\n\
...
refueling_gen$i = refueling_gen[[stop/do_stop_refuel$i, start/do_start_refuel$i], @refueling_false true];\n"
       ...
       printf "lock_automaton0 || generate_gen_automaton0 || generator_ran0 );\n\
