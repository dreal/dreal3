...
\begin{abstract}
We propose reachability analysis techniques for a wide range of hybrid systems based on the use of $\delta$-complete decision procedures. Our method can handle any hybrid systems that can be described by first-order formulas over the reals with computable real functions, which we will call $\lrf$-representable hybrid systems. We show how bounded model checking and invariant validation techniques can be done, guaranteeing $\delta$-completeness, and study the theoretical properties of these problems including decidability and complexity. We show experimental results on nonlinear benchmarks. 
\end{abstract}
