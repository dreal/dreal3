...
\begin{definition}[$\delta$-Perturbations]
...
straightforward. Solving the $\delta$-decision problem is as useful as the standard one, for many
correctness-critical problems. For instance, suppose we perform bounded
...
exponentiation, and trigonometric functions is ``only'' {\sf NP}-complete, in
...
Our main motivation for the tool is to prove lemmas in the Kepler
conjecture, as well as hybrid system verification. In
Table~\ref{tbl:exp}, we show some representative numbers on the scale
of problems that we can solve from the Flyspeck project for the formal
proof of the Kepler Conjecture. It involves formulas with large
