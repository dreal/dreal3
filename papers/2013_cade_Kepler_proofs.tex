...
A pruning step 
$$S\parallel c \Longrightarrow S, (x_i\in I_i')\parallel c$$ 
where $I_i=\beta(S)_i$, corresponds to a proof step
\begin{mathpar}
  \inferrule{
    \forall \vec x (\vec x\in \beta(S,x_i\in I_i') \rightarrow c(\vec x))\\
    \forall \vec x (\vec x\in \beta(S,x_i\in I_i\setminus I_i') \rightarrow
c(\vec x))
  }
  {
    \forall \vec x (\vec x\in \beta(S,x_i\in I_i) \rightarrow c(\vec x))
