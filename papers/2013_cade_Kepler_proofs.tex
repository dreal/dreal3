...
involve large numbers of nonlinear constraints including trigonometric functions. We show
...
correctness in general SMT solvers has been well studied
...
contain a solution is found, or report inconsistency otherwise.
...
well-defined, both procedures never change the
...
scalability. A full table for all the results is given on the tool page.
...
realpaver} (the cosine function around $4\pi/3$). However, our proof checker only involves simple operations written in {\sf OCaml}, and performs the checking completely independent from the solver.
Thus, for the {\sf unsat} answers with proofs, we are
confident about the correctness of the answer, which does not involve
