...
handling nonlinear SMT problems over the
reals~\cite{DBLP:conf/cade/GaoAC12,DBLP:conf/lics/GaoAC12}. We say a decision
...
we can check their correctness using stand-alone symbolic or 
...
that do not carry over the numerical errors, and the complete
...
assignment to all the variables, and updates the assignments based on their
...
been explored in the work~\cite{HySAT}.
...
we show how to validate the generated proofs using a stand-alone proof
checker implementing simple rules and reliable interval arithmetic. The proof
checker interacts with the solver in an abstraction refinement loop to obtain
...
involve large numbers of nonlinear constraints including trigonometric
functions. We show some 
...
Some further comparisons are given in an
extended version~\cite{cade2013extended}. There are now several SMT
...
formulas over the reals.~\cite{CAbook}).
...
\begin{definition}[Encoding Real Numbers~\cite{CAbook}]
...
\end{definition}
\begin{definition}[Computable Real Functions~\cite{CAbook}] We say
...
Let $\delta\in \mathbb{Q}^+$ be a constant and $\varphi$ be a
...
numerically-driven decision procedures. 
...
we follow the format of Abstract DPLL. We will represent ICP
...
As an example, the transformation of the ICP run in Example~\ref{icp-example}
is in ~\cite{cade2013extended}.
...
\end{proposition}
 The proof is contained in the extended report~\cite{cade2013extended}.
\begin{proposition}
...
dReal\footnote{Links to the
tool and benchmarks are on our homepages,
\url{http://www.cs.cmu.edu/~sicung} and
\url{http://www.cs.cmu.edu/~soonhok}}. All the experiments below are run on a
