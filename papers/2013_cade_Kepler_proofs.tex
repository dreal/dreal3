...
contain a solution is found, or report inconsistency otherwise. A high-level
description of ICP is in Algorithm~\ref{algo_icp}. In Algorithm~\ref{algo_icp},
Branch$(B,i)$ is an operator that returns two smaller
...
operators are needed for correctness and
efficiency~\cite{}. More details can be found in~\cite. 
\vspace{-.2cm}
\begin{algorithm}[h!]
...
I_1 \vee \vec x\in \vec I_2))}\mbox{IA}
  }
  }
{
...
\inferrule{
  \inferrule{
		  {\inferrule{
		  {\inferrule{\forall \vec x\in
		  \beta(S_{k'},(x\in I_i)^d,S')\rightarrow \neg c}}
		  { \vdots}
		   }
