...
NRAODESolver::NRAODESolver( const int           i
                            , const char *        n
                            , SMTConfig &         c
                            , Egraph &            e
                            , SStore &            t
...
bool NRAODESolver::icp_prop(rp_problem * p)
{
    rp_selector * select;
    ...
    // rp_problem_destroy(p);
    cerr << "NRAODESolver::icp_prop: " << (result ? "sat" : "unsat") << endl;
...
bool NRAODESolver::icp_solve(rp_problem * p)
{
	rp_selector * select;
	rp_new( select, rp_selector_roundrobin, (p) );
...
variable * NRAODESolver::create_variable( Enode * e )
{
    ...
    cerr << "NRAODESolver::create_variable" << endl;
    cerr << "Name: " << name << "\t"
         << "LB: " << lb << "\t"
         << "UB: " << ub << endl;
    return var;
...
// variable * NRAODESolver::add_variable( Enode * e )
// {
...
//         const double ub = e->getUpperBound();
// 	var -> mk_rp_variable(name, lb, ub);
//         cerr << "NRAODESolver::add_variable" << endl;
//         cerr << "Name: " << name << "\t"
//              << "LB: " << lb << "\t"
//              << "UB: " << ub << endl;
// 	return var;
...
set<variable *> NRAODESolver::get_variables (Enode * e )
{
    set<variable *> result;
    Enode * p = NULL;
...
void NRAODESolver::pop_literal (vector<literal*>::size_type prev_size )
{
...
void NRAODESolver::add_literal ( Enode * e )
{
    ...
    if(_contain_ode) {
        cerr << "NRAODESolver::add_literal _enode_to_vars[" << e << "]" << endl;
...
//
lbool NRAODESolver::inform( Enode * e )
{
    cerr << "inform: " << e << endl;
    assert( e -> isAtom() );
...
//
bool NRAODESolver::assertLit ( Enode * e, bool reason )
{
...
// in EgraphSolver
//
void NRAODESolver::pushBacktrackPoint ( )
{
...
// did not communicate
//
void NRAODESolver::popBacktrackPoint ( )
{
...
//
bool NRAODESolver::check( bool complete )
{
    bool result = true;
...
//
bool NRAODESolver::belongsToT( Enode * e )
{
  (void)e;
  assert( e );
  return true;
...
//
void NRAODESolver::computeModel( )
{
    cerr << "computeModel" << endl;
}

bool NRAODESolver::contain_ode( )
{
    return _contain_ode;
}
...
#ifdef PRODUCE_PROOF
Enode * NRAODESolver::getInterpolants( )
{
  return NULL;
}
#endif
