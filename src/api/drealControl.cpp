...
void checkBarrier(vector<expr>& x, vector<expr>& f, expr& B, double eps) {
...
void checkLyapunov(vector<expr>& x, vector<expr>& f, expr& V, double eps) {
    assert(x.size()==f.size());
    assert(eps>0);
    unsigned n = x.size();
    solver * s = x[0].get_solver();
...
void synthesizeLyapunov(vector<expr*>& x, vector<expr*>& p, vector<expr*>& f, expr& V, double eps) {
...
void synthesizeControlAndLyapunov(vector<expr*>& x, vector<expr*>& p_f, vector<expr*>& p_v, vector<expr*>& f, expr& V, double eps) {
    vector<expr*> p;
    for (auto e : p_f) {
	p.push_back(e);
    }
