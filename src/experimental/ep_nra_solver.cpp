...
#include <utility>
#include <vector>
#include "experimental/ep_nra_solver.h"
#include "./config.h"
...
namespace dreal {
ep_nra_solver::ep_nra_solver(const int i, const char * n, SMTConfig & c, Egraph & e, SStore & t,
...
ep_nra_solver::~ep_nra_solver() {
    DREAL_LOG_INFO << "~ep_nra_solver()";
...
lbool ep_nra_solver::inform(Enode * e) {
    DREAL_LOG_INFO << "ep_nra_solver::inform: " << e;
    // Collect Literal
    m_lits.push_back(e);
    m_need_init = true;
    return l_Undef;
...
bool ep_nra_solver::assertLit(Enode * e, bool reason) {
    DREAL_LOG_INFO << "ep_nra_solver::assertLit: " << e
    ...
    if (e->isDeduced() && e->getPolarity() == e->getDeduced() && e->getDedIndex() == id) {
        DREAL_LOG_INFO << "ep_nra_solver::assertLit: " << e << " is deduced";
        return true;
    }
...
void initialize_nonlinear_constraints(map<pair<Enode*, bool>, shared_ptr<constraint>> & ctr_map,
    ...
    for (Enode * const l : lits) {
        ...
        if (it_nc_pos == ctr_map.end()) {
            ...
            DREAL_LOG_INFO << "ep_nra_solver::initialize_constraints: collect NonlinearConstraint (+): " << *nc_pos;
        ...
        if (it_nc_neg == ctr_map.end()) {
            ...
            DREAL_LOG_INFO << "ep_nra_solver::initialize_constraints: collect NonlinearConstraint (-): " << *nc_neg;
...
void initialize_ode_constraints(map<pair<Enode*, bool>, shared_ptr<constraint>> & ctr_map,
    ...
    for (integral_constraint ic : ints) {
        ...
        DREAL_LOG_INFO << "ep_nra_solver::initialize_constraints: collect ODEConstraint: " << *oc;
...
void ep_nra_solver::initialize_constraints(vector<Enode *> const & lits) {
    ...
    for (Enode * l : lits) {
        ...
        if (l->isIntegral()) {
            ...
            if (it_fc_pos == m_ctr_map.end()) {
                ...
                DREAL_LOG_INFO << "ep_nra_solver::initialize_constraints: collect GenericForallConstraint (+): " << *fc_pos;
            ...
            if (it_fc_neg == m_ctr_map.end()) {
                ...
                DREAL_LOG_INFO << "ep_nra_solver::initialize_constraints: collect GenericForallConstraint (-): " << *fc_neg;
            ...
            DREAL_LOG_FATAL << "ep_nra_solver::initialize_constraints: No Patten";
            throw runtime_error("ep_nra_solver::initialize_constraints: No Patten");
        }
    }
...
void ep_nra_solver::initialize(vector<Enode *> const & lits) {
...
void ep_nra_solver::pushBacktrackPoint() {
    DREAL_LOG_INFO << "ep_nra_solver::pushBacktrackPoint " << m_stack.size();
    if (m_need_init) {
        initialize(m_lits);
        m_need_init = false;
    }
...
void ep_nra_solver::popBacktrackPoint() {
    if (config.nra_use_stat) { config.nra_stat.increase_pop(); }
    DREAL_LOG_INFO << "ep_nra_solver::popBacktrackPoint\t m_stack.size()      = " << m_stack.size();
    m_boxes.pop();
    m_box = m_boxes.last();
...
void ep_nra_solver::handle_sat_case(box const & b) const {
    // SAT
    // --proof option
    if (config.nra_proof) {
...
void ep_nra_solver::handle_deduction() {
...
bool ep_nra_solver::check(bool complete) {
    if (config.nra_use_stat) { config.nra_stat.increase_check(complete); }
    if (m_stack.size() == 0) { return true; }
    DREAL_LOG_INFO << "ep_nra_solver::check(complete = " << boolalpha << complete << ")"
    ...
    DREAL_LOG_INFO << "ep_nra_solver::check: result = " << boolalpha << result;
    ...
    DREAL_LOG_DEBUG << "ep_nra_solver::check(" << (complete ? "complete" : "incomplete") << ") = " << result;
    return result;
}

vector<Enode *> ep_nra_solver::generate_explanation(scoped_vec<shared_ptr<constraint>> const & ctr_vec) {
...
bool ep_nra_solver::belongsToT(Enode * e) {
    (void)e;
    assert(e);
    return true;
}
...
void ep_nra_solver::computeModel() {
    DREAL_LOG_DEBUG << "ep_nra_solver::computeModel" << endl;
    // --model option
