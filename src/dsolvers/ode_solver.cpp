...
ode_solver::ode_solver(int group, SMTConfig& c, set <Enode*> & ode_vars, rp_box b,
                       map<Enode*, int>& enode_to_rp_id, bool & ode_result) :
    _group(group),
    _config(c),
    _ode_vars(ode_vars),
    _b(b),
...
void ode_solver::prune_trajectory(interval& time, IVector& e) {
    ...
    list<pair<interval, IVector>>::iterator ite =
...
IVector ode_solver::extract_invariants(vector<Enode*> vars) {
    ...
    /* Assign current interval values */
    int i = 0;
    for (auto var_ite = vars.begin(); var_ite != vars.end(); var_ite++, i++) {
...
bool ode_solver::simple_ODE() {
    ...
    // ODE_vartype
    var_list.clear();
    for_each(_ode_vars.begin(), _ode_vars.end(), [&] (Enode* ode_var) {
            // If this is _0 variable, then
            if (ode_var->getODEvartype() == l_False) {
                _0_vars.push_back(ode_var);
                _t_vars.push_back(ode_var->getODEopposite());
    ...
    for_each(ode_list.begin(), ode_list.end(), [&funcs, &diff_var] (string& ode_str) {
