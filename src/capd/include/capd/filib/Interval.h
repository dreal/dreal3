...
friend inline Interval nonnegativePart(const Interval &iv){
  if(iv.rightBound() < 0.0)
    throw ::capd::intervals::IntervalError<BoundType>(" Nonnegative part is empty! ", iv.leftBound(), iv.rightBound());
...
friend Interval solveAffineInclusion(const Interval & a,
  ...
  if ( !(a.subset(c)) ) {
    ...
    throw ::capd::intervals::IntervalError<typename Interval::BoundType>("Cannot solve affine inclusion", a.leftBound(), a.rightBound());
...
friend Interval solveAffineInclusion(const Interval & a,
  ...
  Interval t;
  if ( !(a.subset(c)) ) {
    throw ::capd::intervals::IntervalError<typename Interval::BoundType>("Cannot solve affine inclusion", leftBound(a), rightBound(a));
