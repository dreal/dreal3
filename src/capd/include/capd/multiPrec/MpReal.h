...
class MpReal {
  ...
  //  friend MpReal factorial (const unsigned long int e, RoundingMode rnd = MpReal::getDefaultRndMode());
    ...
    friend long int toInt (const MpReal& r, RoundingMode rnd = MpReal::getDefaultRndMode());
    friend long int toLongInt (const MpReal& r, RoundingMode rnd = MpReal::getDefaultRndMode());
    friend unsigned long int toUInt (const MpReal& r, RoundingMode rnd = MpReal::getDefaultRndMode());
...
public:
  ...
  static inline int numberOfDigits(){
    return capd::multiPrec::MpReal::getDefaultPrecision()*std::log(2.0)/std::log(10.0);
  }
