...
PoincareMap<DS,FunT>::operator()(T& originalSet)
{
...
// We move the set to be very close to the section
   T setAfterTheSection = this->reachSection<T>(originalSet);
...
template<typename T>
typename PoincareMap<DS,FunT>::VectorType
PoincareMap<DS,FunT>::operator()(T& originalSet, MatrixType& der)
{
// the originalSet after this function contains a set just ater section
...
template<typename T>
typename PoincareMap<DS,FunT>::VectorType
PoincareMap<DS,FunT>::operator()(T& originalSet, MatrixType& der, MatrixType hessian[])
{
// the originalSet after this function contains a set just ater section
// we move the set close to section
   this->derivativeOfFlow = &der;
...
template<typename T>
typename PoincareMap<DS,FunT>::VectorType
PoincareMap<DS,FunT>::operator()(T& originalSet, CnCoeffType& result)
{
// the originalSet after this function contains a set just ater section
// we move the set close to section
   this->derivativeOfFlow = NULL;
...
template <typename DS, typename FunT>
template<typename T>
T PoincareMap<DS,FunT>::reachSection(T& theSet)
// The common procedure to reach section for all types of the sets.
// As a result we obtain the set closer to section than sectionFactor * sizeOfSet
{
