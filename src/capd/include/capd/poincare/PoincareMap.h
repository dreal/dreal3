...
/// as a good working. It can be changed to a value from the interval (0,1)
  static BoundType timeStepCorrectionFactor;
  static int maxCorrectionTries;            ///< maximal number of correction tries
  static ScalarType minCrossingTimeStep;    ///< minimal time step during section crossing
...
/// (closer than sectionFactor*sizeOfTheSet)
/// It returns set on the section or just after the section (to be used in fuction crossSection)
/// This function is common for all types of C^0, C^1, C^2, C^n sets
  template<typename T>
  T reachSection(T& theSet);
