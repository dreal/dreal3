...
inline unsigned hashkey1 (const word &w)
{
	int len = w. length ();
	if (!len)
		return 13;
	const char *txt = w. text ();
	unsigned code = ((unsigned) (txt [0]) << 6) ^
		((unsigned) (txt [len - 1]));
	if (len > 3)
		code ^= (unsigned) (txt [2]) << 4;
	return code;
} /* word::hashkey1 */

inline unsigned hashkey2 (const word &w)
{
	int len = w. length ();
	if (!len)
		return 7;
	const char *txt = w. text ();
	unsigned code = ((unsigned) (txt [0])) ^
		((unsigned) (txt [len - 1] << 9));
	if (len > 4)
		code ^= (unsigned) (txt [3]) << 4;
	return code;
} /* word::hashkey2 */
