...
int inv_pend() {
    ...
    expr f1 = x2;
    expr f2 = -(-6 * sin(x3) * (x4 ^ 2) + 100 * u - 10 * x2 + 147 * cos(x3) * sin(x3)) /
              (5 * (3 * (cos(x3) ^ 2) - 14));
    expr f3 = x4;
    expr f4 =
        -(-3 * cos(x3) * sin(x3) * (x4 ^ 2) + 343 * sin(x3) + 50 * u * cos(x3) - 5 * x2 * cos(x3)) /
        (3 * (cos(x3) ^ 2) - 14);
    vector<expr *> f = {&f1, &f2, &f3, &f4};
    expr V = x2 * (p1 * x1 + p2 * x2 - p3 * x3 - p4 * x4) +
             x1 * (p5 * x1 + x2 - p7 * x3 - p8 * x4) -
             x3 * (2.63237 * x1 + 3.77814 * x2 - p9 * x3 - 2.12247 * x4) -
             1.0 * x4 * (0.499053 * x1 + 0.697897 * x2 - 2.12247 * x3 - p6 * x4);
