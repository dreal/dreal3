...
optimizer::optimizer(box & b, vector<Enode *> const & l, Egraph & e, SMTConfig & c)
    ...
    for (auto lit : l) {
        ...
        if (pos && lit->isEq()) {
            ...
            f = egraph.mkNum("0");
            f_plain = f;
        } else if ((pos && lit->isLeq()) || (pos && lit->isLt()) || (!pos && lit->isGeq()) ||
                   (!pos && lit->isGt())) {
            f_plain = egraph.mkMinus(left, right);
            f = egraph.mkPlus(f_plain, egraph.mkAbs(egraph.cons(f_plain)));
        } else if ((pos && lit->isGeq()) || (pos && lit->isGt()) || (!pos && lit->isLeq()) ||
                   (!pos && lit->isLt())) {
            f_plain = egraph.mkMinus(left, right);
...
bool optimizer::improve_naive(box & p) {  //  note that p is a point not a nontrivial box
    ...
    for (auto v : vars) {
        ...
        double step = ((p_v - domain.get_value(v).lb()) / (length + delta)) *
                      (domain.get_value(v).ub() - p_v);
