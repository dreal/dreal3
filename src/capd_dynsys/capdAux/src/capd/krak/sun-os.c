...
/*  XSetStandardProperties(display, win, argv[0],argv[0],NULL,
    argv, argc, &size_hints);  NULL zastapilem 0 */

   XSetStandardProperties(display, win, argv[0],argv[0],0,argv, argc, &size_hints);
...
/*----------------------------- define stipples -----------------------------*/
   ...
   for (i=0;i<17;i++) stipple[i]=
      XCreateBitmapFromData(display,RootWindow(display,screen),(char *)patt_pntr[i],8,8);
