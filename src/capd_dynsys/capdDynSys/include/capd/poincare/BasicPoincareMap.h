...
//  typedef typename DS::CurveType CurveType;
  ...
  VectorType crossSection(VectorType beforeSection, VectorType v, ScalarType & lastStep);
  ...
  void turnOnStepControl()   {  m_stepControl = true;  }
  void turnOffStepControl()  {  m_stepControl = false; }

protected:
  template<class SetType>
  void makeOneStep(SetType& s)     {  m_dynamicalSystem(s);  }
  void makeOneStep(VectorType & x) {  x = m_dynamicalSystem(const_cast<const VectorType & >(x));  }

  void setFirstTimeStep(const VectorType & v);
  void setNextTimeStep(const VectorType & v);
...
public:
  ...
  ~SaveStepControl(){
  m_ds.setStep(m_step);
  m_ds.setStepChangeAllowance(m_stepChangeAllowance);
  }
protected:
  DS & m_ds;
  ScalarType m_step;
...
// setFirstTimeStep
///
/// if step control is turned on then it sets first time step for the integration
////////////////////////////////////////////////////////////////////////////////////////////
template <typename DS, typename FunT>
inline void BasicPoincareMap<DS, FunT>::setFirstTimeStep(const VectorType & v){
  if(this->m_stepControl){
    ScalarType firstStep = this->m_dynamicalSystem.getFirstTimeStep(v, m_dynamicalSystem.getStep());
...
// setNextTimeStep
///
/// if step control is turned on then it sets next time step for the integration
////////////////////////////////////////////////////////////////////////////////////////////
template <typename DS, typename FunT>
inline void BasicPoincareMap<DS, FunT>::setNextTimeStep(const VectorType & v){
  if(this->m_stepControl){
    ScalarType step = this->m_dynamicalSystem.getFirstTimeStep(v, 10 * m_dynamicalSystem.getStep());
