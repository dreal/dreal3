...
{
  if(values.dimension() != this->m_dim - this->m_indexOfFirstParam){
    throw std::runtime_error("BasicFunction::setParameters : numbers of parameters are not equal! ");
  }

  int cnt= this->m_indexOfFirstParam;
  int i =0;
  while(cnt < this->m_dim){
      this->m_val[this->m_order*cnt] = values[i];
      ++cnt;
      ++i;
  }
}
