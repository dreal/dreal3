...
let process_flow ~k ~q (modemap:Modemap.t) : (flows_annot * Basic.formula) =  
  let m = Map.find q modemap in
  let mode_formula = make_mode_cond ~k ~q in
  let flows_annot =
    List.map
      (fun ode ->
        (k, q, Ode.subst (make_variable k "") ode)
      )
      m.flows
  in
  let inv_formula = match (Mode.invs_op m) with
    ...
    | Some invs ->
      ...
      Basic.make_and (mode_formula::invt_conds)
  in
...
let process_step (varmap : Vardeclmap.t) (modemap : Modemap.t) (step:int) : Basic.formula = 
  ...
  let jump_flow_part = 
    List.map 
      (fun q -> 
        try
          let (_, flow_for_q) = process_flow ~k:step ~q modemap in
...
let build_flow_annot_list (h : Hybrid.t) (step:int) = 
  ...
  let num_of_modes = Enum.count (Map.keys modemap) in
  let list_of_modes = List.of_enum ( 1 -- num_of_modes ) in
  let list_of_steps = List.of_enum ( 0 -- step ) in
  let list_of_k_q = List.cartesian_product list_of_steps list_of_modes in
  let flow_annot_list = 
    List.flatten
      (List.map 
         (function (k, q) -> 
           let (flows_annot, _) = process_flow ~k ~q modemap in
           flows_annot
         )
         list_of_k_q
      )
...
let compile_ode_definition (h : Hybrid.t) k = 
  let {init_id; init_formula; varmap; modemap; goals} = h in
  let num_of_modes = calc_num_of_mode modemap in
  let flows_annot = build_flow_annot_list h k in
  let odes =
    List.map
      (fun (k, q, (x, e)) ->
        let ode_group  = k * num_of_modes + q in
        (ode_group, x, e)
      )
      flows_annot 
  in
  List.map (fun (g, x, e) -> DefineODE (g, x, e)) odes
