...
let split (x : string) (e1 : t)  (e2 : t) =
  let {Intv.low = l1; Intv.high = h1} = Map.find x e1 in
  let {Intv.low = l2; Intv.high = h2} = Map.find x e2 in
  Map.add x {Intv.low = h2; Intv.high = h1} e1
